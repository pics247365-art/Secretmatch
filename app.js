...
function renderCard(){const st=document.getElementById('cardStage');if(index>=deck.length){show('results');return;}const c=deck[index];document.getElementById('progressText').textContent=(index+1)+' / '+deck.length;document.getElementById('progressFill').style.width=Math.round((index/deck.length)*100)+'%';st.innerHTML='<article id="activeCard" class="play-card"><div><div class="ill">'+c.icon+'</div><div class="meta">'+c.cat+'</div><h2 class="card-title">'+c.title+'</h2><p class="desc">'+c.desc+'</p><div class="tags">'+c.tags.map(t=>'<span class="tag">'+t+'</span>').join('')+'</div><p class="meta">ימינה רוצה · שמאלה לא · למעלה אולי</p></div><div class="choices"><button class="choice choose-yes" onclick="answer(\''+c.id+'\',\'yes\')">רוצה</button><button class="choice choose-maybe" onclick="answer(\''+c.id+'\',\'maybe\')">אולי</button><button class="choice choose-stop" onclick="answer(\''+c.id+'\',\'skip\')">לא</button></div><div class="choices"><button class="choice" onclick="goBack()">חזור</button><button class="choice" onclick="answer(\''+c.id+'\',\'skip\')">דלג</button></div></article>';swipe(c.id);}
...
function answer(id,val){history.push({id,val,index});answers[id]=val;localStorage.setItem('secretMatchAnswers',JSON.stringify(answers));index++;renderCard();}
function goBack(){const last=history.pop();if(!last)return;delete answers[last.id];localStorage.setItem('secretMatchAnswers',JSON.stringify(answers));index=Math.max(0,last.index);renderCard();}
...
document.getElementById('startBtn').onclick=()=>{shuffle();show('game')};document.getElementById('resetBtn').onclick=()=>{answers={};localStorage.removeItem('secretMatchAnswers');shuffle();show('home')};document.getElementById('backBtn').onclick=()=>show('home');document.getElementById('againBtn').onclick=()=>{shuffle();show('game')};document.querySelectorAll('.nav').forEach(b=>b.onclick=()=>show(b.dataset.screen));
